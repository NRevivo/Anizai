...
| KG-A-17 | **OOM mechanism unresolved.** In the 7D cloud window `LOG_INFO_SAMPLE_RATE=1.0` was the differentiating variable across three TaskManager crash attempts at constant memory+burst (2560Mi and 6Gi both OOMKilled with it; 2560Mi survived without it), flat JVM heap placing growth Python-side. But the "~100× records over the Beam logging channel" story does NOT hold if INFO is unsampled either way (empirically it is, on the Flink UDF path). Candidate: the var changes which handlers `setup_logging()` installs (duplicate propagation, not a higher rate). Unverified. **The day-run (2560Mi, 9 sources, 24h) was Prometheus-clean → NOT KG-A-9.** | Phase 7D (2026-07-27) | Medium | Reproduce in isolation; instrument `setup_logging()` handler installation under 1.0 vs absent. |

...
- **HackerNews archival consequence of Phase 7D D1a (2026-07-27):** the HN dedup key is now derived from `story_id` alone (`sha256("hackernews|<story_id>")`), and `social_vault` archival is gated by the same `exists_by_content_hash()` check as enrichment. So `social_vault` now retains **only the FIRST comment-set captured per HN story** — the raw discussion archive stops updating on re-fetch, not just enrichment ("one enrichment per story" and "one archive per story" share the key). Revisiting via a refresh-window key (D1 option c) would restore both. Tracked KG-A-16.
...
| `filter_rejects.canonical_event_id` (Phase 7D T7) | New instance-key column (TEXT, nullable) on `filter_rejects`, populated at both drop branches (global_news + the new HackerNews social branch) from `_cost_trace_id(record)` — joins a reject back to its `llm_cost_events` / vault rows. Pre-7D rows read NULL (no backfill). Added in migration 004 + `init.sql`, index `idx_filter_rejects_cei`. Tracked KG-A-13 (closed 2026-07-27). |
| `filter_rejects` ~2× growth from HN reject capture (Phase 7D) | With `REJECT_CAPTURE_ENABLED=true` the HackerNews social path writes reject rows (day-run reference ~2,868 HN rejects/day — an **instance** count: a persistent front-page low-signal story is re-captured every 20-min pulse). HN reject bodies store joined top-comment texts, so capture roughly **doubles** `filter_rejects` growth. Trivial for a short window; **confirm Postgres PVC headroom before a multi-day run.** Live 2026-07-27 slice: 288 HN + 119 newsapi + 978 arxiv reject rows. Tracked KG-A-19. |

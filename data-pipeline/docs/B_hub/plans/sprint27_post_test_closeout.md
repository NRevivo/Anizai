...
|------|--------|-------------|---------|----------------|
| 27.1 | `[ ]` | Close KG-B-4 (Origin KG-PHASE8-7) — replace `logging.basicConfig()` in `agent/worker.py` with the standard `setup_logging()` from `utils/logging_config.py`. Verify INFO-level logs appear correctly. | — | §8.8.2 |

...
| flink-jobmanager / -taskmanager | Deployment | `anizai-flink:1.19.1-7d` (Phase 7D T11 deploy, 2026-07-27 — new tag, `-7b5i` never overwritten); K8s HA enabled (Phase 9 follow-up, 2026-05-19). **The 2026-07-26 `replicas: 0` hold is superseded** — T11 §8.3 step 6 applied both manifests, which declare `replicas: 1`, and that is what brought Flink up for the window (git and live now agree for these two; the KG-C-10 drift persists for `telegram` / `polymarket`). Note the manifest edits were left **uncommitted** during the window; the durable set lands in the teardown commit. **Deployed digest for `1.19.1-7d` = `sha256:9a73a780…ebced303`.** New image built + pushed 2026-08-01, **not yet rolled out**: **`1.19.1-pmcov`, digest `sha256:963cbf9d…c0238c52`** — carries the Track-S Silver catalog carry-through. **A Flink rebuild is mandatory for that sprint, not optional:** `processing/silver_job.py` is baked into this image, so without it a cancel+resubmit would recompile the OLD mapper, reach RUNNING, and silently drop every added field. **KG-A-15 checked on this rebuild: `pip freeze` inside `1.19.1-pmcov` is byte-identical to `1.19.1-7d` — 118 packages, zero dependency movement** (compared by digest, not tag) | 7D T11 (2026-07-27); `1.19.1-pmcov` pushed 2026-08-01 |
...
| polymarket | Deployment | **Live Deployment references `anizai-polymarket:0.3.0-price`** (digest `sha256:2ae00dae…899cac8`, pushed 2026-07-30) — the collaborator's price fix. **This row said `0.2.0-p95` until 2026-08-01: the tag was never recorded here and existed only in a gitignored file on his machine.** Rollback tag `0.2.0-p95` = digest `sha256:a2a3e82e…e93cfd5c`. New coverage image built + pushed 2026-08-01, **not yet rolled out**: `0.4.0-coverage`, digest `sha256:1d27ec05…6c43d758`. On main-pool; comments feature-flagged off. **Held at `replicas: 0` live (manifest says 1 — KG-C-10)** | 9.5-B; hold verified 2026-07-26; tag corrected + `0.4.0-coverage` pushed 2026-08-01 |

...
| KG-C-15 | (new — 2026-08-16) | **`polymarket` is OOMKilled at its 512Mi limit — first surfaced on `0.4.1-inactive`'s first multi-day run.** Container restarted with `OOMKilled`, `exit=137` at `2026-08-16T02:29:16Z`, ~11h after start. Steady-state usage measured immediately after at **373Mi against a 512Mi limit (73%)**, with `requests: 128Mi`. The hourly discovery sweep holds ~2,600 nested markets in memory while filtering down to ~1,220 collectable, so the peak sits well above the resting figure | **No data lost so far** — the restart completed inside the hour and the 02:00 sweep still landed 1,224 rows; 19 consecutive hourly sweeps with Bronze == `momentum_vault` at 23,276. The exposure is that a restart landing *mid-sweep* would drop that hour, and at ~11h between kills this recurs roughly twice a day for the rest of any long run | Raise `resources.limits.memory` on `k8s/producers/polymarket-deployment.yaml` (512Mi → 1Gi is the obvious first step) and raise `requests` off 128Mi, which is far below observed steady state. **Manifest change + pod restart — deferred to Ron, not applied mid-run.** ⚠️ **Do NOT apply the Flink OOM lesson here.** `bringup_profiles.md` §5 trap 3 states that container memory *cannot* fix the Flink TaskManager OOM, because that failure is JVM direct-buffer exhaustion bounded by `taskmanager.memory.*` rather than by cgroups. This is the opposite case: a genuine cgroup OOMKill (`OOMKilled=true`, `exit=137`) of a pure-Python process, where raising the limit **is** the correct fix. Confusing the two would either waste effort here or repeat the 6Gi mistake there. **Medium** | 2026-08-16, during the PolymarketBronzeStale investigation | Before the next long unattended window; pair with any `anizai-polymarket` manifest edit |
